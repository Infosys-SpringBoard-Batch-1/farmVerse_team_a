...
// Curated map of crop names → specific high-quality Unsplash photo IDs (verified)
const CROP_IMAGES = {
...
// Fallback pool for unknown crops — beautiful farm/field shots
...
function getCropImage(cropName) {
  ...
  // Hash crop name to pick a consistent fallback image
  let hash = 0;
  for (let i = 0; i < (key?.length || 0); i++) hash = key.charCodeAt(i) + hash; return FALLBACK_IMAGES[Math.abs(hash) % FALLBACK_IMAGES.length]; } export default function ViewFarm() { const { id } = useParams(); const navigate = useNavigate(); const [farm, setFarm] = useState(null); const [loading, setLoading] = useState(true); useEffect(() => { loadFarm(); }, []); const loadFarm = async () => { try { const response = await getFarmById(id); console.log("VIEW FARM RESPONSE:", response); console.log("CROPS:", response.farm.crops); setFarm(response.farm); } catch (err) { console.error(err); } finally { setLoading(false); } }; const handleDeleteCrop = async (cropId) => {
    if (window.confirm("Are you sure you want to delete this crop?")) {
      try {
        await deleteCrop(cropId);
        alert("Crop deleted successfully");
        loadFarm(); // Reload farm details
      } catch (err) {
        console.error(err);
  ...
  return (
    <DashboardLayout>
      {/* Back Button */}
      <button onClick={() => navigate("/farm")} className="flex items-center gap-2 text-gray-500 hover:text-emerald-600 transition-colors mb-6 font-semibold" >
        <FaArrowLeft />
        Back to Farms
      </button>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
        ...
        <div className="flex justify-between items-center mt-12 mb-8">
          ...
          <button onClick={() => navigate(`/farm/${farm.farmId}/crop/add`)} className="bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-semibold px-6 py-3 rounded-xl flex items-center gap-2 shadow-lg shadow-green-600/30 transition-all hover:scale-105" >
            <FaPlus />
            Add Crop
          </button>
        </div>
        ...
        {(farm.crops ?? []).length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8"> {(farm.crops ?? []).map((crop) => (
              <div key={crop.cropId} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group" >
                {/* Automatic Crop Image */}
                <div className="overflow-hidden">
                  <img src={getCropImage(crop.cropName)} alt={crop.cropName} className="w-full h-52 object-cover group-hover:scale-110 transition-transform duration-500" />
                </div>
                ...
                <div className="flex border-t border-gray-100 divide-x divide-gray-100 ">
                  <button onClick={() => navigate(`/farm/${farm.farmId}/crop/edit/${crop.cropId}`)} className="flex-1 py-4 text-blue-600 font-semibold hover:bg-blue-50 flex justify-center items-center gap-2 transition-colors" >
                    <FaEdit /> Edit
                  </button>
                  <button onClick={() => handleDeleteCrop(crop.cropId)} className="flex-1 py-4 text-red-600 font-semibold hover:bg-red-50 flex justify-center items-center gap-2 transition-colors" >
                    <FaTrash /> Delete
                  </button>
                </div>
              </div>
