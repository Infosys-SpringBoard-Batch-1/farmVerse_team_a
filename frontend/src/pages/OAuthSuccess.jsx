import { useEffect, useRef } from "react"; import { useNavigate } from "react-router-dom"; function parseJwt(token) { try { const base64Url = token.split(".")[1]; if (!base64Url) return null; const base64 = base64Url.replace(/-/g, "+").replace(/_/g, "/"); return JSON.parse(window.atob(base64)); } catch (error) { console.error("JWT Parse Error:", error); return null; } } export default function OAuthSuccess() { const navigate = useNavigate(); const hasRun = useRef(false); useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-100 flex items-center justify-center">
      <div className="bg-white p-10 rounded-3xl shadow-xl flex flex-col items-center max-w-sm w-full mx-4 relative overflow-hidden">
        ...
        <h2 className="text-2xl font-bold text-gray-800 tracking-tight">Authenticating</h2>
        <p className="text-gray-500 mt-2 text-center font-medium">Please wait while we sign you in...</p>
      </div>
    </div>
  );
}